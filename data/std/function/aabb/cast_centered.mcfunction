...
# save params
$data modify storage minecraft:std local_aabb_params.selector set value '$(selector)'
$data modify storage minecraft:std local_aabb_params.dx set value $(dx)
$data modify storage minecraft:std local_aabb_params.dy set value $(dy)
$data modify storage minecraft:std local_aabb_params.dz set value $(dz)
$data modify storage minecraft:std local_aabb_params.cmd set value '$(cmd)'

# get origin offset
execute \
    store result storage minecraft:std local_aabb_params.origin_offset_x \
    double -0.5 \
    run data get storage minecraft:std local_aabb_params.dx
execute \
    store result storage minecraft:std local_aabb_params.origin_offset_y \
    double -0.5 \
    run data get storage minecraft:std local_aabb_params.dy
execute \
...
execute \
    ...
    run data get storage minecraft:std local_aabb_params.dx 1000
execute \
    store result score $dy std_local_aabb \
    run data get storage minecraft:std local_aabb_params.dy 1000
execute \
...
execute \
    if score $greater_than_1_components std_local_aabb matches 3 \
    run execute store result storage minecraft:std local_aabb_params.dx \
    double 0.001 \
    run scoreboard players get $dx std_local_aabb
execute \
    if score $greater_than_1_components std_local_aabb matches 3 \
    run execute store result storage minecraft:std local_aabb_params.dy \
    double 0.001 \
    run scoreboard players get $dx std_local_aabb
execute \
    if score $greater_than_1_components std_local_aabb matches 3 \
...
data remove storage minecraft:std local_aabb_params
